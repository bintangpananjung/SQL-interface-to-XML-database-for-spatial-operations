...
function rebuildFromTree(tree: Select, data: any, columnNames: Set<string>) {
  ...
  selectTree.columns = listColumns;
...
function rebuildTree(
  ...
  for (const data of dataList) {
    const columns = mapColumnsPerTable.has(data.as)
      ...
      : new Set<string>();
    const treePerFrom = rebuildFromTree(oldTree, data, columns);
...
export { rebuildWhere, rebuildTree, rebuildFromTree };
